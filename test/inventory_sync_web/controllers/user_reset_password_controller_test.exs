defmodule InventorySyncWeb.UserResetPasswordControllerTest do
  ...
  describe "GET /users/reset-password" do
    test "renders the reset password request page", %{conn: conn} do
      ...
      assert response =~ "Enter your email to receive reset instructions"
    end
  end
  ...
  describe "GET /users/reset-password/:token" do
    ...
    test "renders reset password form", %{conn: conn, token: token} do
      ...
      response = html_response(conn, 200)
      assert response =~ "Set new password"
    end

    test "does not render reset password with invalid token", %{conn: conn} do
      conn = get(conn, ~p"/users/reset-password/invalid")
      assert redirected_to(conn) == ~p"/users/log-in"
